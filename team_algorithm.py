...
class Statistics:
    ...
    @staticmethod
    def Enable():
        Statistics._use = True
    
    @staticmethod
    def IsEnabled():
        return Statistics._use

    @staticmethod
    def Add( name : str, value = 1):
        if Statistics._use:
            if name in Statistics._stats:
                Statistics._stats[name] += value
            else:
                Statistics._stats[name] = value
...
class Utils:
    ...
    @staticmethod
    def DenormalizeAngle(angle : float):
    ...
    @staticmethod
    def DenormalizeAngles(angles : numpy.ndarray):
    ...
    def GetPoint2PlaneDistance(pos : Pos2, angle):
        ...
        return abs(math.sin(ang2) * dist)
    
    @staticmethod
    def GetSASDistance(angle : float, a : float, b : float):
        return math.sqrt(a * a + b * b - 2 * math.cos(angle) * a * b)
    ...
    @staticmethod
    def GetTargetAxleState2D(targetPos2D : Pos2) -> list:
        dist = targetPos2D.Distance()
        if dist > Params.Arm2D1 + Params.Arm2D2: # Unreachable!
            Statistics.Add("GTAS Unreachable")
            return [0.2, 0.5, 0]
        rotV1 = Utils.GetAngleFromPosition(targetPos2D) + Utils.GetSSSAngle(Params.Arm2D2, Params.Arm2D1, dist)
        rotV2 = Utils.GetSSSAngle(dist, Params.Arm2D1, Params.Arm2D2)
        rotV3 = (1 * math.pi - rotV1 - rotV2) / math.pi / 2
        return [Utils.NormalizeAngle(rotV1), Utils.NormalizeAngle(rotV2), rotV3]

    @staticmethod
    def Get2DPositionFromAxleState(rotation : numpy.ndarray) -> Pos2:
        angles = Utils.DenormalizeAngles(rotation)
        disx = Utils.GetSASDistance(angles[1], Params.Arm2D1, Params.Arm2D2)
        rdx = Utils.GetSSSAngle(Params.Arm2D2, Params.Arm2D1, disx)
        ahx = angles[0] - rdx
        return Pos2([disx * math.cos(ahx), disx * math.sin(ahx)])

    @staticmethod
    def GetPositionFromAxleState(rotation : numpy.ndarray) -> Pos3:
        p2 = Utils.Get2DPositionFromAxleState(rotation[1:3])
        rotH = Utils.DenormalizeAngle(rotation[0])
        x = p2.X * math.cos(rotH)
        y = p2.X * math.sin(rotH)
        return Pos3([x, y, p2.Y])
    
    @staticmethod
    def MeasureStateTransformationSteps(current, target) -> float:
...
class StrategyDirect(Strategy):
    ...
    def GetArm2ObstacleDistance(self, targetPos : Pos3, obstaclePos : Pos3):
        ...
        dist = Utils.GetPoint2PlaneDistance(obstaclePos.XY, rotH)
        targetDistH = targetPos.XY.Distance()
        pdis = Utils.GetRectangularDistance([targetDistH - 0.245, targetPos.Z - 0.05])
        if (pdis > Params.Arm2D1 + Params.Arm2D2):
            return -1
        return dist
...
class StrategyAlternate(Strategy):
    ...
    def GetArm2ObstacleDistance(self, targetPos : Pos3, obstaclePos : Pos3):
        ...
        pdis = Utils.GetRectangularDistance([distEq - 0.16, targetPos.Z - 0.05])
        if (pdis > Params.Arm2D1 + Params.Arm2D2):
            return -1
        return dist
...
class MyCustomAlgorithm(BaseAlgorithm):
    ...
    def DetermineStratrgy(self, axleState, targetPos : Pos3, obstaclePos : Pos3, steps):
        ...
        for i in range(steps):
            area = 0
            if pos.X < posL:
                if self.Strategies[1].GetArm2ObstacleDistance(pos, obstaclePos) < 0: continue
                targetRot = self.Strategies[1].GetTargetAxleRotation(pos, obstaclePos)
                area = 0
                pass # use stratrgy alternate or side
            elif pos.X > posR:
                if self.Strategies[0].GetArm2ObstacleDistance(pos, obstaclePos) < 0: continue
                ...
                continue
            stepsEst = Utils.MeasureStateTransformationSteps(axleState, targetRot) + 12
            if i > stepsEst:
                self.SelectedStrategy = 0
                self.StaticStepLimit = stepsEst
                self.DynamicStrategySequence = [ 1 if area == 0 else 0, 0, 1, 2]
                Statistics.Add("SSA{} SEL".format(area))
                self.TargetRot = targetRot
                Statistics.Add("S0 Usage", 1)
                return
    ...
    def GetTargetAxleState(self, axleState, targetPos : Pos3, obstaclePos : Pos3):
        if self.SelectedStrategy == 0 and self.StaticStepLimit > self.Steps: # Static strategy
            return self.TargetRot
        else: # Dynamic strategy
            # curPos = Utils.GetPositionFromAxleState(axleState)
            if self.Steps < 60:
                scale = 0.6 / targetPos.Y 
                targetPos.X *= scale
                targetPos.Y *= scale
                # if self.Steps > 120:
                    # pass
                targetPos.Z = max(targetPos.Z, obstaclePos.Z + 0.3)
            for i in self.DynamicStrategySequence:
                strategy = self.Strategies[i]
                if strategy.GetArm2ObstacleDistance(targetPos, obstaclePos) > 0:
                    Statistics.Add("DS{} SEL".format(i))
                    return strategy.GetTargetAxleRotation(targetPos, obstaclePos)
            Statistics.Add("DSS FAIL", 1)
            return None
    ...
    def GetAction(self, axleState, targetPos : Pos3, obstaclePos : Pos3):
        ...
        self.Steps += 1
        action = [0, 0, 0, 0, 0, 0]
        target = self.GetTargetAxleState(axleState, targetPos, obstaclePos)
        if target == None:
            return action
        self.ArmStable = True
        for i in range(6):
            action[i] = Utils.GetAxleRotationTransformation(axleState[i], target[i])
            if self.Steps < 150 and i != 0 and action[i] > 0.5:
                self.ArmStable = False
        return action
    ...
    def NotifyRoundEnd(self, result):
        Statistics.Add("S{} Score".format(self.SelectedStrategy), result[4])
        # self.Statistics[self.Strategy][1] += result[-1]
        pass

    def NotifyTestEnd(self):
        if Statistics.IsEnabled():
            stats = Statistics.GetRawData()
